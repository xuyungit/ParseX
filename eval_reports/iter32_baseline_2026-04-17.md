...
- Documents: 16
- Avg edit distance: 0.244
- Avg char F1: 0.897
- Avg heading F1: 0.544
- Avg table cell F1: 0.487
- Total warnings: 32
- API calls (OCR/VLM/LLM): 32/52/8
- LLM fallback hits: 14
- Total wall time: 570.6s

## Per Document

| Document | Edit Dist | Char F1 | Heading F1 | Table F1 | Warn | API O/V/L | Fallback | Time |
|----------|-----------|---------|------------|----------|------|-----------|----------|------|
| deepseek | 0.077 | 0.939 | 0.000 | 0.000 | 0 | 0/0/0 | 0 | 0.7s |
| ocr01 | 0.249 | 0.925 | 0.556 | 0.903 | 1 | 4/1/1 | 3 | 38.0s |
| ocr_scan_jtg3362 | 0.224 | 0.893 | 0.286 | 0.638 | 3 | 4/6/1 | 0 | 35.3s |
| paper01 | 0.260 | 0.979 | 0.854 | 0.000 | 6 | 6/13/0 | 0 | 61.2s |
| paper_chn01 | 0.704 | 0.711 | 0.788 | 1.000 | 3 | 5/5/1 | 1 | 169.9s |
| paper_chn02 | 0.635 | 0.809 | 0.129 | 0.000 | 5 | 3/3/1 | 1 | 52.9s |
| patent01 | 0.373 | 0.909 | 0.500 | 1.000 | 1 | 2/17/1 | 1 | 44.0s |
| pdf_text01_tables | 0.041 | 0.979 | 0.000 | 0.992 | 0 | 0/0/0 | 0 | 56.1s |
| receipt | 0.094 | 0.957 | 1.000 | 0.000 | 2 | 1/2/0 | 0 | 2.6s |
| simple_doc01 | 0.711 | 0.458 | 0.286 | 0.000 | 10 | 0/0/1 | 2 | 3.0s |
| text_code_block | 0.050 | 0.975 | 0.500 | 0.000 | 0 | 0/0/0 | 0 | 0.6s |
| text_pic02 | 0.240 | 0.951 | 0.222 | 0.267 | 0 | 6/3/1 | 3 | 49.5s |
| text_report01 | 0.207 | 0.889 | 0.588 | 1.000 | 1 | 0/2/1 | 3 | 3.6s |
| text_table01 | 0.004 | 0.998 | 1.000 | 0.000 | 0 | 0/0/0 | 0 | 0.0s |
| text_table_libreoffice | 0.006 | 0.997 | 1.000 | 1.000 | 0 | 0/0/0 | 0 | 47.6s |
| text_table_word | 0.035 | 0.984 | 1.000 | 1.000 | 0 | 1/0/0 | 0 | 5.7s |
| **Average** | **0.244** | **0.897** | **0.544** | **0.487** | **32** | **32/52/8** | **14** | **570.6s** |
...
|--------------|-------|
| Orphan heading | 17 |
| Low-confidence VLM | 5 |
| Duplicate body text | 4 |
| Heading level jump | 4 |
| Table count mismatch | 1 |
| Text volume drift | 1 |

## Warning Hotspots

- simple_doc01: 10 warning(s) — Page 1: heading level jump from H2 to H4 (1.3.1).; Page 1: orphan H4 heading without H3 parent (1.3.1).; ...
- paper01: 6 warning(s) — Page 3: low-confidence VLM description (distance=0.89).; Page 5: low-confidence VLM description (distance=0.75).; ...
...
- ocr_scan_jtg3362: 3 warning(s) — Page 1: low-confidence VLM description (distance=0.97).; Page 2: orphan H2 heading without H1 parent (前言).; ...
- paper_chn01: 3 warning(s) — Page 1: heading level jump from H1 to H3 (Abstract).; Page 1: orphan H3 heading without H2 parent (Abstract).; ...
- receipt: 2 warning(s) — Page 1: heading level jump from H1 to H3 (账单与付款).; Page 1: orphan H3 heading without H2 parent (账单与付款).
- ocr01: 1 warning(s) — Table count mismatch: document has 3, markdown rendered 2.
- patent01: 1 warning(s) — Page 2: orphan H2 heading without H1 parent (1. 一种基于静载试验识别连续梁桥实际刚度的方法，其特征在于，包括如下步骤：).
- text_report01: 1 warning(s) — Rendered text volume drifted beyond tolerance: source=857 chars, output=1045 chars.
...
- Themes: `output_light, extra_heading`
- Blocks: extra=11, missing=11
- Output-only excerpt: `### （1）本文方法只需对中间支座的竖向反力增量
进行测量。目前已有商用的特种支座（装备了应变传感单元）可直接提供竖向支座反力数据。实桥的
尺寸越大、限于条件可承受的测点数越少，则本文方法的优势越突出。
（2）本文方法数据处理过程简单，计算量小。
（3）采用区间作为几何度量单位，便于分析和计算。虽然定位的直接结果是1对小区间，但结合检
测设备可方便地确定损伤情形。
（4）损伤定位的效果可通过改变荷载F和虚拟区间长度m的取值来进行调节。F值越大、m值越
小，则效果越好。
（5）本文的分析是在一维框架内进行的，后续研究可将分析扩展到二维情形，从而充分考虑…`
...
- Themes: `output_heavy, missing_table_shape`
- Blocks: extra=15, missing=17
- Output-only excerpt: `抗凝药
依度沙班
遵循医嘱减少依度沙班的药量；停用 Paxlovid 三天后，依度沙班可用回原剂量
华法林
可能会影响患者的凝血功能，增加出血风险，应密切监测凝血功能，如有异常及时处理
抑郁
曲唑酮
根据病情下调曲唑酮的剂量，并密切监测副作用；停用 Paxlovid 三天后，曲唑酮可用回原剂量
失眠
唑吡坦
密切监测副作用；若病情需要，可减助眠药的药量；若药量有调整，可在停用 Paxlovid 三天后用回原剂量
佐匹克隆
密切监测副作用；若病情需要，可减助眠药的药量；若药量有调整，可在停用 Paxlovid 三天后用回原剂量
焦虑
阿普唑仑
...
- Blocks: extra=9, missing=17
- Output-only excerpt: `> [图片] A blue Chinese company logo with a stylized green-and-blue V-shaped emblem above the text.
...
- Blocks: extra=14, missing=14
- Output-only excerpt: `> [图片] A small line drawing showing a slanted, cross-like mechanical or structural shape with a curved line beneath it.
### 公路钢筋混凝土及预应力
### 混凝土桥涵设计规范
> [图片] 公路 涵 标准
...
- Blocks: extra=5, missing=5
- Output-only excerpt: `3.参考以下图示装订方式，采用“骑马钉”方式装订成册。
> [图片] A close-up photo of several bound booklet or folder edges stacked together. The visible colors are black, green, pink, and white.
> [图片] 示意图展示了册页装订方式，左侧有“骑马钉”标注和箭头指向装订位置。`
...
- Output-only excerpt: `> [图片] A rounded rectangular card or wallet icon on a dark background, with layered colored bands at the top and a red center shape.
Apply and use in minutes<sup>2</sup>
1. 3% savings is earned as Daily Cash and is the total amount of Daily Cash earned for these purchases. You c…`
