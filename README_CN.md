...
**Provider（输入层）** — 从 PDF（PyMuPDF 字符级提取）或 DOCX（Docling OOXML 解析）中提取原始内容。每个文本片段都携带字体名、字号和加粗标记 — 这些元数据驱动下游的标题检测，无需 LLM。
...
| 换行修复 | 🚧 进行中 | 跨行句子拼接 |
| 章节检测 LLM 兜底 | 🚧 进行中 | 用于规则无法覆盖的文档 |
| 公式提取 | 🚧 进行中 | LaTeX 输出，需集成模型 |
| 幻觉检测 | 🚧 进行中 | VLM 输出与原生文本交叉验证 |
| 阅读顺序 | 🚧 进行中 | 多栏布局支持 |
