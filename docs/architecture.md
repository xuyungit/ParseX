...
选择逻辑（逐区域决策）：
  ...
  - 矢量渲染文字（原生提取为空但有渲染内容）→ 全页 OCR
    ∘ 检测信号：表格 header row ≥50% 空单元格（≥3 列），
      表明 Word 导出等场景将文字渲染为矢量曲线（bezier paths）
    ∘ 实现：`pipeline._check_page_quality()` 将页面重分类为 SCANNED，
      OCRBuilder 自动重跑全页，同时恢复表头和周围标题文本
...
✅ 已实现 — VLM Review Processor（页面级审校，`processors/vlm_review.py`）：
  ...
  - 触发条件（选择性，非全量）：
    ...
    - 含矢量渲染文本的 NATIVE 页面（由 `_check_page_quality()` 重分类
      为 SCANNED 后自动进入审校范围）
  - fix_table 应用逻辑：全文替换（非前缀替换），避免截断摘要导致的
    内容重复；对跨页合并表格（corrected 长度 < original 的 50%）
    自动跳过，防止丢失其他页的表格行
