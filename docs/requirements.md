...
**状态（2026-04-08）**：HeaderFooterProcessor 已实现确定性检测（几何位置 + 跨页频率），首页 identity 保留已收紧为最多 2 个元素（按信息密度排名），保留元素标记 `exclude_from_heading_detection`。页码检测覆盖阿拉伯数字、罗马数字、中文"第 N 页"等格式。剩余问题：不规则页眉（左右页交替）和置信度低时的 LLM fallback 尚未启用。
...
**状态（2026-04-08）**：逐页分类已实现。新增 OCR-layered scan 检测，通过空间关系（文本是否在大图内部）识别"可搜索扫描件"，避免误分类为 NATIVE。

### P16: OCR-layered scanned PDFs（可搜索扫描件）

**发现时间**：2026-04-08

部分 PDF 是扫描底图 + 预先 OCR 生成的文本层的组合。文本层可能是可见的（覆盖在底图上）或不可见的（用于全文检索）。这类 PDF 有以下特征：

- PyMuPDF 能提取到文本（来自 OCR 文本层），因此被误分类为 NATIVE
- 提取的文本质量很差（旧版 OCR 的错误，如"混疑土"应为"混凝土"）
- OCR 不会重跑（因为页面被认为是 NATIVE）
- VLM 也无法有效参与（进入 supplement 模式丢弃 table/description）

**关键检测信号**：大图覆盖页面 >50% 且 >70% 的文字在该图 bbox 内。这是一个纯几何信号，不依赖字体名、文本质量等细碎特征。

**已实现方案**：`_classify_page()` 中增加空间检测，将此类页面正确分类为 SCANNED。OCR Builder 在重新 OCR 前移除旧的 native text 元素。

**验证结果**：JTG 3362-2018（268 页扫描件）264/268 页正确检出，14 个正常 PDF 零误报。

**后续改进（2026-04-08）**：VLM Review Processor 已实现，SCANNED 页面自动触发页面级 VLM 审校，可修正重新 OCR 后仍存在的识别错误（`fix_text` 修正）。待端到端验证。

### P17: 矢量渲染文本丢失

**发现时间**：2026-04-08

Word/WPS 导出 PDF 时，部分文字（尤其是 CJK 标题和表头）被转换为矢量曲线（outlined text），而非嵌入字体字符。这导致：

- PyMuPDF 文本提取为空（字符已变成贝塞尔曲线路径）
- 没有 image 元素产生（矢量图不是位图）
- VLM 没有处理入口

**代表案例**：`text_table_word.pdf` 第 2 页表头"专家评审组名单"等文字由 593 条贝塞尔曲线构成。PyMuPDF `find_tables()` 能检测到表格框线（矢量线段），但表头单元格提取为空。

**状态（2026-04-09）**：已通过 OCR 重跑解决。`pipeline._check_page_quality()` 检测表格 header row ≥50% 空单元格（≥3 列）的 NATIVE 页面，重分类为 SCANNED 后由 OCRBuilder 全页重新提取。OCR 能可靠恢复表头文字和周围标题。text_table_word: table_cell_f1 0.913→1.000，char_f1 0.973→0.987。VLM Review 在 OCR 恢复后运行，判断无需修正，返回 "ok"。

### P18: DOCX 流式文档处理路径缺失

**发现时间**：2026-04-08

DOCX 是"流式"文档，有样式语义（标题样式、列表类型、表格结构）但没有页面几何信息（bbox）。当前 pipeline 将 PDF 的几何处理逻辑（HeaderFooterProcessor、ContentValueProcessor、CodeBlockProcessor 等基于 bbox 的处理器）强行套到 DOCX 上，导致严重的信息丢失：

- DOCX 元素的 bbox 全为 `(0, 0, 0, 0)`，导致 ContentValueProcessor 的 `wide_sparse_banner` 和 `image_cluster` 惩罚全面误触发
- 绝大多数短文本被错误标记为 `skip_render`，封面、目录、子标题、列表项等关键内容全部丢失
- 同理，HeaderFooterProcessor 的跨页重复检测和 CodeBlockProcessor 的等宽字体检测均无意义

**代表案例**：`text_report01`（四川省装备首台套申请模板），71 个元素完整提取后仅有个位数元素通过 ContentValueProcessor 过滤，输出 701 chars（实际应 1017 chars），触发 text volume drift 告警。

**状态（2026-04-08）**：已修复。pipeline 在 DOCX 模式下跳过几何依赖处理器（HeaderFooter、CodeBlock、ContentValue），同时跳过无意义的 builder 步骤（MetadataBuilder 字体统计、OCR、ReadingOrder）。DOCX 处理链简化为：Extract → Chapter → Table → Image → Formula → LineUnwrap → TextClean → Render。

### P19: 不支持 .doc 格式

**发现时间**：2026-04-08

旧版 Word 文档（.doc）在企业场景中仍然常见，但 ParserX 此前仅支持 .docx。

**状态（2026-04-08）**：已修复。通过 LibreOffice `soffice --convert-to docx` 前置转换 .doc → .docx，保留语义信息后走 DOCX 处理路径。
